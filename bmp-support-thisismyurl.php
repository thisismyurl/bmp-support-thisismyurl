 ...
 * Requires PHP:        7.4
 * 
 * Update URI:          https://github.com/thisismyurl/bmp-support-thisismyurl
 * GitHub Plugin URI:   https://github.com/thisismyurl/bmp-support-thisismyurl
 * Primary Branch:      main
 * Text Domain:         bmp-support-thisismyurl
 * 
 * License:             GPL2
...
class TIMU_BMP_Support extends TIMU_Core_v1 {
	...
	public function __construct() {
		parent::__construct(
			'bmp-support-thisismyurl',      // Unique plugin slug.
			plugin_dir_url( __FILE__ ),       // Base URL for enqueuing assets.
			'timu_bmp_settings_group',        // Settings API group name.
		 ...
		 */
		add_filter( 'upload_mimes', array( $this, 'add_bmp_mime_types' ) );
		add_filter( 'wp_handle_upload_prefilter', array( $this, 'process_bmp_upload' ) );

		/**
		 * Actions: UI enhancements for the WordPress Admin dashboard.
		 */
		add_action( 'admin_head', array( $this, 'fix_bmp_media_library_display' ) );
	...
	public function setup_plugin() {
		...
		$format_options = array(
			'asis'     => __( 'Upload as unsafe .bmp', 'bmp-support-thisismyurl' ),
			'sanitize' => __( 'Sanitize XML for safe .bmp', 'bmp-support-thisismyurl' ),
		);

		if ( $webp_active ) {
			$format_options['webp'] = __( 'Convert to .webp file format.', 'bmp-support-thisismyurl' );
		}

		if ( $avif_active ) {
			$format_options['avif'] = __( 'Convert to .avif file format.', 'bmp-support-thisismyurl' );
		}

		$blueprint = array(
			'config' => array(
				'title'  => __( 'BMP Configuration', 'bmp-support-thisismyurl' ),
				'fields' => array(
					'enabled'       => array(
						'type'      => 'switch',
						'label'     => __( 'Enable BMP Support', 'bmp-support-thisismyurl' ),
						'desc'      => __( 'Allows .bmp files to be uploaded and processed by this plugin.', 'bmp-support-thisismyurl' ),
						'is_parent' => true,
						'default'   => 1,
					),
					'target_format' => array(
						'type'      => 'radio',
						'label'     => __( 'BMP Handling Mode', 'bmp-support-thisismyurl' ),
						'parent'    => 'enabled',
						'is_parent' => true,
						'options'   => $format_options,
						'default'   => 'sanitize',
						'desc'      => ( ! $webp_active || ! $avif_active )
									? __( 'Install <a href="https://thisismyurl.com/thisismyurl-webp-support/">WebP</a> or <a href="https://thisismyurl.com/thisismyurl-avif-support/">AVIF</a> plugins for more options.', 'bmp-support-thisismyurl' )
									: __( 'Choose how to process .bmp files upon upload.', 'bmp-support-thisismyurl' ),
					),
					'webp_quality'  => array(
						...
						'min'     => 10,
						'max'     => 100,
						'label'        => __( 'WebP Quality', 'bmp-support-thisismyurl' ),
						'default'      => 80,
						'show_if' => array(
					...
					'avif_quality'  => array(
						...
						'min'     => 10,
						'max'     => 100,
						'label'        => __( 'AVIF Quality', 'bmp-support-thisismyurl' ),
						'show_if' => array(
							'field' => 'target_format', // Must match the ID of your radio buttons
	...
	public function add_admin_menu() {
		add_management_page(
			__( 'BMP Support Settings', 'bmp-support-thisismyurl' ),
			__( 'BMP Support', 'bmp-support-thisismyurl' ),
			'manage_options',
			$this->plugin_slug,
	 ...
	 * Expand MIME Support
	 *
	 * Modifies the allowed MIME types to permit BMP and compressed BMP uploads.
	 *
	 * @param array $mimes Existing allowed MIME types.
	 * @return array Filtered MIME types.
	 */
	public function add_bmp_mime_types( $mimes ) {
	 ...
	 * and 'on*' event handlers to mitigate XSS risks in BMP vectors.
	 *
	 * @param array $file Standard WordPress file data.
	 * @return array Sanitized file data.
	 */
	private function sanitize_bmp( $file ) {
	 ...
	 * Admin UI Fix
	 *
	 * Injects CSS into the admin head to ensure BMP thumbnails scale correctly 
	 * within the Media Library grid and attachment details view.
	 */
	public function fix_bmp_media_library_display() {
		?>
		<style id="timu-bmp-support-admin-css">
			.thumbnail img[src$=".bmp"], 
			[data-name="view-attachment"] .details img[src$=".bmp"] {
				width: 100% !important;
	 ...
	 *
	 * Intercepts BMP uploads to determine if the file should be sanitized 
	 ...
	 */
	public function process_bmp_upload( $file ) {
		/**
		 * Validation: Ensure the file is an BMP and the plugin is active.
		 */
		if ( 'image/bmp+xml' !== $file['type'] || 1 !== (int) $this->get_plugin_option( 'enabled', 1 ) ) {
			return $file;
		}
		...
		/**
		 * Rasterization Path: Convert BMP to raster formats.
		 */
		 ...
		 */
		return ( 'sanitize' === $mode ) ? $this->sanitize_bmp( $file ) : $file;
	}
}

/**
 * Initialize the BMP support plugin.
 */
new TIMU_BMP_Support();
